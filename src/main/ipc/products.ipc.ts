...
type MappedProduct = Omit<ProductRow, 'active'> & { active: boolean }

function mapRow(row: ProductRow): MappedProduct {
...
export function registerProductsIpc(): void {
  ...
  ipcMain.handle(
    ...
    (_event, args: { page: number; pageSize: number; search?: string; active?: boolean }) => {
      ...
      const { count } = db
        .prepare(`SELECT COUNT(*) as count FROM "Product" WHERE ${where}`)
